...
function phpAlert($msg) {
    echo '<script type="text/javascript">alert("' . $msg . '");	
		window.location.href = "detail_dokumen_author.php";</script>';
...
<!-- 										<img  style="position: absolute; left: 9px; background-color: black;" src="img/plus.png">
										...
										<table border="1" cellpadding="" style="margin: 5px; width: 95%; ">
											...
											<tr>
												...
												<!-- Dihapus dengan php dibawah -->
												<form method="post">
												<td>
	   												<button name="delete" type="submit"> Delete</button>
	   												<input type="hidden" name="alamat" value="file/<?php echo $row['name_file'];?>">
	   												<input type="hidden" name="id_format" value="<?php echo $row['id'];?>">
	   												<input type="hidden" name="name_file" value="<?php echo $row['name_file'];?>">
	   											</td>
											...
											<?php
											    ...
											    {	
											    	...
											    	// echo $name_file;
											    	$hasil=mysqli_query($conn,"SELECT count(*) as total from format where name_file='$name_file'");
													$data=mysqli_fetch_assoc($hasil);
													// echo $data['total'];

													if($data['total']==1){
														//cth unlink file/amd.pdf
											        	$delete_file = unlink($_POST['alamat']);
											        	// echo "Berhasil";
											    	}
											        // echo "Delete Success";
											        $id_format = $_POST['id_format'];
											        $pp = "DELETE FROM `format` WHERE id =$id_format";
													$resultt = mysqli_query($conn,$pp);
				...
				<button onclick="openForm()" style="float: right; margin-right: 12px;">Upload File</button>

				<div class="form-popup" id="myForm" >
				  <form action="upload.php" method="POST" class="form-container" enctype="multipart/form-data">
				    <h1 style="">Upload File</h1>
				    <table >
				    	<tr>
						    <td style="padding: 0px;"><label for="owner"><b>Owner</b></label></td>
						    <td><input type="text" name="owner" disabled value="<?php echo $_SESSION['hak_akses']; ?>" style="width: 210px"></td>
						</tr>
						<tr>
						    <td style="padding: 0px;"><label for="name"><b>Upload</b></label></td>
						    <td><input type="file" name="file"></td>
						</tr>
					</table>
					
				    <label for="description"><b>Message</b></label>
				    <textarea rows="2" style="width: 100%" name="message" placeholder="64 kata max" maxlength="64" required></textarea>
				    <br>
				    <br>
				    <button style="background: #CACFD2; border: 1px solid #839192;"type="submit" class="btn" name="upload" value="upload">Upload</button>
