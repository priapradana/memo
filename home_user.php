										...
										<div id="myTabContent" class="tab-content">
										    ...
										    <tr>
										  	...
										  	if(isset($_GET['search'])){
												...
												// $result = mysqli_query($conn,"SELECT * FROM document WHERE name LIKE '$search' and category_class = 'public'  ");
												
												$result = mysqli_query($conn,"SELECT document.id, document.id_project, document.name, document.message, document.category_class, document.fk_id_user FROM document INNER JOIN project ON project.category_class='public' and document.category_class='public' and document.id_project = project.id and document.name LIKE '$search' ");
												$total = mysqli_num_rows($result);
												$pages = ceil($total/$halaman);            
												$result2 = mysqli_query($conn,"SELECT document.id, document.id_project, document.name, document.message, document.category_class, document.fk_id_user FROM document INNER JOIN project ON project.category_class='public' and document.category_class='public' and document.id_project = project.id and document.name LIKE '$search' LIMIT $mulai, $halaman");
												$no =$mulai+1;
											  ...
											  $page = isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
											  $mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;
											  $result = mysqli_query($conn,"SELECT document.id, document.id_project, document.name, document.message, document.category_class, document.fk_id_user FROM document INNER JOIN project ON project.category_class='public' and document.category_class='public' and document.id_project = project.id ");
											  $total = mysqli_num_rows($result);
											  $pages = ceil($total/$halaman);            
											  $result2 = mysqli_query($conn,"SELECT document.id, document.id_project, document.name, document.message, document.category_class, document.fk_id_user FROM document INNER JOIN project ON project.category_class='public' and document.category_class='public' and document.id_project = project.id LIMIT $mulai, $halaman");
											  $no =$mulai+1;
										  	}
											...
											foreach ($result2 as $row) {
												...
												// if ($category_class == 'public'){
													...
													<tr>
														<td><?php echo $no++;?></td>
														<td><a href="detail_dokumen_user.php?id_dokumen=<?php echo $row['id'];?>"><?php echo $row['name'];?></a></td>
