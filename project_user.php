	...
	<body class="">
		...
		<header id="header">
			<div id="logo-group">
				...
				<form action="project_user.php" class="header-search pull-right">
...
<!-- 										<img  style="position: absolute; left: 9px; background-color: black;" src="img/plus.png">
										...
										<table border="1" cellpadding="" style="margin: 5px; width: 95%; ">
										    ...
										    if(isset($_GET['search'])){
												...
												$result = mysqli_query($conn,"SELECT * FROM project WHERE name LIKE '$search' and category_class='public' ");
												$total = mysqli_num_rows($result);
												$pages = ceil($total/$halaman);            
												$result2 = mysqli_query($conn,"SELECT * FROM project WHERE category_class='public' and name LIKE '$search' LIMIT $mulai, $halaman");
												$no =$mulai+1;
											  ...
											  $page = isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
											  $mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;
											  $result = mysqli_query($conn,"SELECT * FROM project WHERE category_class='public'");
											  $total = mysqli_num_rows($result);
											  $pages = ceil($total/$halaman);            
											  $result2 = mysqli_query($conn,"SELECT * FROM project WHERE category_class='public' LIMIT $mulai, $halaman");
											  $no =$mulai+1;
										  	}
										    ...
										    <tr>
										        <td><?php echo $no++;?></td>
												<td style="width: 13%;"><a href="project_detail_user.php?var=<?php echo $row['id'];?>"><?php echo $row['name'];?></a></td>
